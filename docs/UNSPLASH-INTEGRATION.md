...
```env
NEXT_PUBLIC_UNSPLASH_ACCESS_KEY=g9-mDed-mP4Btd5ZNHAxPTQnIX5zcLBX6AkIDY2l1WM
```

**Note:** This is a public access key (safe to expose in client-side code)
...
export function CarCard({ car }: { car: Car }) {
  ...
  const images = car.imageUrls.length > 0
    ? car.imageUrls
...
export default async function CarDetailPage({ params }: { params: { id: string } }) {
  ...
  const images = car.images?.length > 0
    ? car.images
    : getCarImages(car.make, car.model);
...
export const KOREAN_CAR_MODELS = {
  Hyundai: [
    ...
    'Palisade', 'Kona', 'Venue', 'Ioniq 5',
    'Ioniq 6', 'Grandeur'
  ],
  Kia: [
    'Sportage', 'Sorento', 'Telluride', 'Seltos',
    'Soul', 'Forte', 'K5', 'Carnival', 'EV6', 'Niro'
  ],
  Genesis: [
...
const thumbnail = optimizeUnsplashUrl(baseImages[0], { width: 400, quality: 70 });
